...
# ==============================================================================
# 批量路径规划脚本 - Gemini Flash 3 Preview 版本
# ==============================================================================
# 用途: 对 gemini-3-flash-preview 目录下的所有场景进行批量路径规划
# 修改日期: 2026-01-13
# ==============================================================================

BASE_DIR="/home/pjlab/fbh/LabUtopia"
BASE_OUTPUT_DIR="$BASE_DIR/outputs/path_planning_batch_results_gemini_flash"

# 可配置参数：障碍物膨胀半径（米）
# 可以根据需要修改这个值，或通过环境变量覆盖:
#   OFFSET_RADIUS=0.3 bash run_batch_path_planning_gemini_flash.sh
OFFSET_RADIUS=${OFFSET_RADIUS:-0.3}  # 默认 0.3 米（与 YAML 配置一致）

# 场景目录和 goal_pairs 目录配置
SCENES_BASE_DIR="$BASE_DIR/gemini-3-flash-preview"  # 修改点1: 场景目录
GOAL_PAIRS_BASE_DIR="$BASE_DIR/outputs/gemini_flash_nav_targets_1_16_15_31/goal_pairs"  # 修改点2: goal_pairs 目录

# 通用 barrier 图片（当场景目录没有 barrier 图片时使用）
...
echo "========================================"
echo "批量路径规划 - Gemini Flash 3 Preview"
echo "开始时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo "输出目录: $OUTPUT_DIR"
echo "障碍物膨胀半径: ${OFFSET_RADIUS} 米"
echo "场景目录: $SCENES_BASE_DIR"
echo "Goal Pairs 目录: $GOAL_PAIRS_BASE_DIR"
echo "========================================"
echo ""
...
for scene_dir in "$SCENES_BASE_DIR"/*/; do
  ...
  # ------------------------------------------------------------
  # 查找 Barrier 图片 (修改点3: 改进的图片查找逻辑)
  # ------------------------------------------------------------
  # 策略1: 优先使用场景目录中的 PNG 文件
  ...
  # ------------------------------------------------------------
  # 构建 Goal Pairs 文件路径 (修改点4: 新的路径格式)
  # ------------------------------------------------------------
  ...
  if [ -n "$barrier_img" ] && [ -f "$goal_pairs" ]; then
    ...
    echo "Goal pairs: $(basename "$goal_pairs")"
    echo "========================================"
...
")
    ...
    [ -z "$barrier_img" ] && echo "  - No barrier image found"
    [ ! -f "$goal_pairs" ] && echo "  - No goal_pairs file: $goal_pairs"
    echo ""
  fi
done
