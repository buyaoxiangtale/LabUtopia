...
class NavigationTaskNew(BaseTask):
    """
    顺序式导航任务：
    - 支持外部传入多个有序起终点对（队列）
    - 每次到达后自动切换到下一个起终点
    ...
    def step(self) -> Dict[str, Any]:
        ...
        # 检查是否需要连续设置下一个路径点（当reset_needed为False但有剩余任务时）
        if not self.reset_needed and self._has_remaining_goals():
            # 尝试设置下一个路径点，如果失败则回退到重置模式
            if not self._apply_next_goal_until_success():
                print("Warning: 连续任务设置失败，触发重置")
    ...
    def on_task_complete(self, success: bool) -> None:
        """到达后切换到下一个起终点。如果相邻任务连接点相同，则连续执行。"""
        if success and self._has_remaining_goals():
            # 检查当前终点是否与下一个起点相同
            current_end = np.array(self.current_end)
            next_start = np.array(self.goal_pairs[self.current_goal_idx + 1][0])

            # 如果两点足够接近（阈值0.1米），则连续执行
            if np.linalg.norm(current_end - next_start) < 0.1:
                next_task_num = self.current_goal_idx + 2  # +1因为还没推进，+1因为显示的是任务编号
                print(f"\n🔗 连续执行检测：当前任务终点与任务 {next_task_num} 起点相同")
                print(f"   连接点: [{current_end[0]:.2f}, {current_end[1]:.2f}]")
                print(f"   跳过重置，直接开始任务 {next_task_num}")
                # 推进队列指针但不重置，直接继续
                self.current_goal_idx += 1
                # 不设置reset_needed，继续执行
                return
    ...
    def _normalize_pairs(self, pairs: List[Any]) -> List[Tuple[list, list]]:
        ...
        for item in pairs:
            if isinstance(item, dict) and "start" in item and "end" in item:
                normalized.append((list(item["start"]), list(item["end"])))
            elif (
                isinstance(item, (list, tuple))
                and len(item) == 2
                and all(isinstance(p, (list, tuple)) for p in item)
            ):
